...
public:
	...
	void stepHighway(double egoVelocity, long long timestamp, int frame_per_sec, pcl::visualization::PCLVisualizer::Ptr& viewer, std::ofstream& radarDataFile, std::ofstream& lidarDataFile)
	{
		...
		for (int i = 0; i < traffic.size(); i++)
		{
			...
			if(trackCars[i])
			{
				...
				tools.lidarSense(traffic[i], viewer, timestamp, visualize_lidar, lidarDataFile);
				tools.radarSense(traffic[i], egoCar, viewer, timestamp, visualize_radar, radarDataFile);
