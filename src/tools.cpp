...
lmarker Tools::lidarSense(Car& car, pcl::visualization::PCLVisualizer::Ptr& viewer, long long timestamp, bool visualize, std::ofstream& lidarDataFile)
{
    ...
    car.ukf.ProcessMeasurement(meas_package, lidarDataFile);

    return marker;
}

// sense where a car is located using radar measurement
rmarker Tools::radarSense(Car& car, Car ego, pcl::visualization::PCLVisualizer::Ptr& viewer, long long timestamp, bool visualize, std::ofstream& radarDataFile)
{
	...
	// std::cout << "=========== "<<car.name <<" processmeasurement start " << "==============" <<std::endl;
    car.ukf.ProcessMeasurement(meas_package, radarDataFile);
	// std::cout << "=========== "<<car.name <<" processmeasurement ends " << "==============" <<std::endl;
    return marker;
}
...
void Tools::ukfResults(Car car, pcl::visualization::PCLVisualizer::Ptr& viewer, double time, int steps)
{
	...
	if(time > 0)
	{
		...
		while(ct <= time)
		{	
			// std::cout << "=========== "<<car.name <<" Prediction start " << "==============" <<std::endl;
			ukf.Prediction(dt);
			// std::cout << "=========== "<<car.name <<" Prediction ends " << "==============" <<std::endl;
